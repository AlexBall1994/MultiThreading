...
public class AuctionServer
{
	...
	private Object submitItemLock = new Object();
	...
	public int submitItem(String sellerName, String itemName, int lowestBiddingPrice, int biddingDurationMs)
	{
		...
		synchronized(submitItemLock){
			...
			int numberOfItems = (!itemsPerSeller.containsKey(sellerName)) ? 0 : itemsPerSeller.get(sellerName);
			itemsAndIDs.put(lastListingID, newItem);
			itemsPerSeller.put(sellerName, ++numberOfItems);
			itemsUpForBidding.add(newItem);

			//System.out.println(lastListingID + "size:"+itemsUpForBidding.size());
	...
	public List<Item> getItems()
	{
		...
		synchronized(submitItemLock){
			List<Item> newItemL = new ArrayList<Item>();

			newItemL.addAll(itemsUpForBidding);
	...
	public boolean submitBid(String bidderName, int listingID, int biddingAmount)
	{
		...
		synchronized(submitBidLock){
			...
			if (highestBidders.containsKey(listingID)){
				...
				int itemsPerB = itemsPerBuyer.get(oldbidder);
				itemsPerB--;
				itemsPerBuyer.put(oldbidder, itemsPerB);

				highestBidders.put(listingID, bidderName);
				int amount = !itemsPerBuyer.containsKey(bidderName) ? 0 : itemsPerBuyer.get(bidderName);
				amount++;
				itemsPerBuyer.put(bidderName, amount);
	...
	public int checkBidStatus(String bidderName, int listingID)
	{
		...
		synchronized(submitItemLock){
			if (!itemsAndIDs.containsKey(listingID))
				...
				Item itemSelling = itemsAndIDs.get(listingID);

				if (itemSelling.biddingOpen())
					return 2;
				else if (!highestBidders.containsKey(listingID))
					return 3;
				else if(highestBidders.get(listingID) == bidderName){
					int amount = itemsPerBuyer.get(bidderName);
					amount--;
					itemsPerBuyer.put(bidderName, amount);
					itemsUpForBidding.remove(itemSelling);
					int sellAmount = itemsPerSeller.get(itemSelling.seller());
					sellAmount--;
					itemsPerSeller.put(itemSelling.seller(), sellAmount);
					soldItemsCount++;
	...
	{
		synchronized(submitBidLock){
			return highestBidders.containsKey(listingID);
		}
