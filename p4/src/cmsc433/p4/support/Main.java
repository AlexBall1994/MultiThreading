...
public class Main {
	...
	public static void main(String[] args) {
		...
		System.out.printf("nil is %s; reference count is %d%n", lops.exportList(nil), lops.getReferenceCount(nil));

		// Cons 3 onto null actor. "[3]" should be printed; reference count for
		// list3 should be 0.
		ActorRef list3 = lops.cons(3, nil);
		//ActorRef list4 = lops.cons(4, nil);
		System.out.printf("list3 is %s; reference count is %d%n", lops.exportList(list3),
				lops.getReferenceCount(list3));

		// Get reference count of nil (= tail of list3). "1" should be printed.
		System.out.printf("Reference count for nil is %d%n", lops.getReferenceCount(nil));
		...
		System.out.printf("Tail for list3 is %s; reference count for tail is %d%n", lops.exportList(tailList3),
				lops.getReferenceCount(tailList3));
		System.out.printf("Reference count for nil is %d%n", lops.getReferenceCount(nil));
		...
		System.out.printf("Reference count for list23 is %d; reference count for list3 is %d%n",
		...
		System.out.printf("Head of list 23 is %s; tail of list23 is %s%n", lops.head(list23),
				lops.exportList(lops.tail(list23)));
		System.out.printf("Reference count for list3 is %d%n", lops.getReferenceCount(list3));
		...
		System.out.printf("Reference count for list123 is %d%n", lops.getReferenceCount(list123));
		System.out.printf("Reference count for tail(list123) is %d%n", lops.getReferenceCount(lops.tail(list123)));

		// Now try map. "[]", "[4]" and "[2,3,4]" should be printed. Printed
		// reference counts should all be 0

		ActorRef mappedNil = lops.add1(nil);
		System.out.printf("mappedNil is %s; reference count is %d%n", lops.exportList(mappedNil),
				lops.getReferenceCount(mappedNil));

		ActorRef mappedList3 = lops.add1(list3);

		System.out.printf("mappedList3 is %s; reference count is %d%n", lops.exportList(mappedList3),
				lops.getReferenceCount(mappedList3));
		ActorRef mappedList123 = lops.add1(list123);
		System.out.printf("mappedList123 is %s; reference count is %d%n", lops.exportList(mappedList123),
		...
		System.out.printf("Reference count of tail(list3) is %d%n", lops.getReferenceCount(lops.tail(list3)));
		...
		System.out.printf("The reference count for tail(list123) is %d%n", lops.getReferenceCount(lops.tail(list123)));
		ActorRef list11 = lops.cons(null
				, nil);
