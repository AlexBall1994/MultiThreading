...
public class IntegerListOperations {
	 ...
	 */
	public ActorRef nil() {
		Future<Object> fmsg = Patterns.ask(listManager, new NullActorRequestMessage(), futureDelay);
		try {
			NullActorResultMessage msg = (NullActorResultMessage) Await.result(fmsg, awaitDelay);
	...
	public ActorRef cons(Integer head, ActorRef tail) {
		Future<Object> fmsg = Patterns.ask(listManager, new ConsActorRequestMessage(head, tail), futureDelay);
		try {
			ConsActorResultMessage msg = (ConsActorResultMessage) Await.result(fmsg, awaitDelay);
	...
	public Boolean isNull(ActorRef list) {
		Future<Object> fmsg = Patterns.ask(list, new NullStatusRequestMessage(), futureDelay);
		try {
			NullStatusResultMessage msg = (NullStatusResultMessage) Await.result(fmsg, awaitDelay);
	...
	public Integer head(ActorRef list) {
		Future<Object> fmsg = Patterns.ask(list, new HeadRequestMessage(), futureDelay);
		try {
			HeadResultMessage msg = (HeadResultMessage) Await.result(fmsg, awaitDelay);
			return (msg.getHead());
		} catch (Exception e) {
	...
	public ActorRef tail(ActorRef list) {
		Future<Object> fmsg = Patterns.ask(list, new TailRequestMessage(), futureDelay);
		try {
			TailResultMessage msg = (TailResultMessage) Await.result(fmsg, awaitDelay);
			return (msg.getTail());
		} catch (Exception e) {
	...
	public Integer getReferenceCount(ActorRef list) {
		Future<Object> fmsg = Patterns.ask(list, new ReferenceCountRequestMessage(), futureDelay);
		try {
			ReferenceCountResultMessage msg = (ReferenceCountResultMessage) Await.result(fmsg, awaitDelay);
	...
	public ActorRef importList(List<Integer> list) {
		...
		try {
			ImportListResultMessage msg = (ImportListResultMessage) Await.result(fmsg, awaitDelay);
	...
	public <T> T collect(IntegerCollector<T> collector, ActorRef list) {
		...
		try {
			CollectResultMessage<T> msg = (CollectResultMessage<T>) Await.result(fmsg, awaitDelay);
	...
	public ActorRef map(IntegerCompute f, ActorRef list) {
		...
		Future<Object> fmsg = Patterns.ask(listManager, req, futureDelay);
		try {
			MapResultMessage msg = (MapResultMessage) Await.result(fmsg, awaitDelay);
